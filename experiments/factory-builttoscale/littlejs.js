...
function ASSERT(...assert) {
  enableAsserts && console.assert(...assert);
  if(!(assert[0]) && !TRACEFIRED){
    TRACEFIRED = true;
    console.trace("but where...");
  }
}
...
class Vector2
{
    ...
    /** Returns a copy of this vector offset by the x and y values passed in
...
class EngineObject
{
    ...
    constructor(pos=vec2(), size=vec2(1), tileInfo, angle=0, color, renderOrder=0)
    {
        ...
        /** @type {EngineObject?} - Parent of this object */
        this.parent;
    ...
    update()
    {
        ...
        if (parent)
        {
          if(!this.enableParenting) return;
    ...
    addChild(child, localPos=vec2(), localAngle=0)
    {
        ...
        child.pos = child.localPos.multiply(vec2(this.getMirrorSign(),1)).rotate(-this.angle).add(this.pos);
        child.angle = this.getMirrorSign()*child.localAngle + this.angle;
    }
